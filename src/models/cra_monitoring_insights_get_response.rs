/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.688.6
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// CraMonitoringInsightsGetResponse : CraMonitoringInsightsGetResponse defines the response schema for `cra/monitoring_insights/get`
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct CraMonitoringInsightsGetResponse {
    /// A unique identifier for the request, which can be used for troubleshooting. This identifier, like all Plaid identifiers, is case sensitive.
    #[serde(rename = "request_id")]
    pub request_id: String,
    /// A unique ID identifying a User Monitoring Insights Report. Like all Plaid identifiers, this ID is case sensitive.
    #[serde(rename = "user_insights_id")]
    pub user_insights_id: String,
    /// An array of Monitoring Insights Items associated with the user.
    #[serde(rename = "items")]
    pub items: Vec<models::CraMonitoringInsightsItem>,
}

impl CraMonitoringInsightsGetResponse {
    /// CraMonitoringInsightsGetResponse defines the response schema for `cra/monitoring_insights/get`
    pub fn new(request_id: String, user_insights_id: String, items: Vec<models::CraMonitoringInsightsItem>) -> CraMonitoringInsightsGetResponse {
        CraMonitoringInsightsGetResponse {
            request_id,
            user_insights_id,
            items,
        }
    }
}

