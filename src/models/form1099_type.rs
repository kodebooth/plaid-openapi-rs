/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.688.6
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// Form1099Type : Form 1099 Type
/// Form 1099 Type
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Form1099Type {
    #[serde(rename = "FORM_1099_TYPE_UNKNOWN")]
    Form1099TypeUnknown,
    #[serde(rename = "FORM_1099_TYPE_MISC")]
    Form1099TypeMisc,
    #[serde(rename = "FORM_1099_TYPE_K")]
    Form1099TypeK,

}

impl std::fmt::Display for Form1099Type {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Form1099TypeUnknown => write!(f, "FORM_1099_TYPE_UNKNOWN"),
            Self::Form1099TypeMisc => write!(f, "FORM_1099_TYPE_MISC"),
            Self::Form1099TypeK => write!(f, "FORM_1099_TYPE_K"),
        }
    }
}

impl Default for Form1099Type {
    fn default() -> Form1099Type {
        Self::Form1099TypeUnknown
    }
}

